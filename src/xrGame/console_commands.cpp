...
#include "../xrengine/xrServerInfo.h"
...
static void full_memory_stats	( )
{
	...
	Msg		("* [x-ray]: crt heap[%d K], process heap[%d K], render[%d K]",_crt_heap/1024,_process_heap/1024,_render/1024);
#endif // SEVERAL_ALLOCATORS
...
class CCC_ServerInfo : public IConsole_Command
{
public:
	CCC_ServerInfo(LPCSTR N) : IConsole_Command(N)  {	
		bEmptyArgsHandled = TRUE;
	};
	virtual void Execute(LPCSTR args) {
		...
		SHORT server_info_size = (SHORT)server_info.Size();
		Msg("===================================================");
		for( SHORT i=0; i<server_info_size; ++i )
		{			
			Msg(server_info[i].name);
		}
		Msg("===================================================");
	}
};
...
public:
	...
	virtual void Execute(LPCSTR args) {
		...
		string_path		fn;
		FS.update_path	(fn, "$app_data_root$", fn_);
	...
	CCC_DemoPlay(LPCSTR N) : 
	  ...
	  virtual void Execute(LPCSTR args) {
		  if (0==g_pGameLevel)
		  {
			  ...
			  FS.update_path	(fn, "$app_data_root$", fn);
...
{\
static CCC_RadioMask x##CCC_RadioMask1(p1,p2,p3);		pConsoleCommands->AddCommand(&x##CCC_RadioMask1);\
static CCC_RadioMask x##CCC_RadioMask2(p4,p5,p6);		pConsoleCommands->AddCommand(&x##CCC_RadioMask2);\
...
public		:
	...
	virtual void	Execute	(LPCSTR args)
	{
		...
		CObject* obj			= Level().CurrentViewActor();	VERIFY(obj);
...
void CCC_RegisterCommands()
{
	...
	CMD1(CCC_ServerInfo,		"server_info"			);
...
/////////////////////////////////////////////HIT ANIMATION END////////////////////////////////////////////////////
	...
	CMD4(CCC_Float,		"ph_timefactor",				&phTimefactor				,			0.000001f	,1000.f			);
	CMD4(CCC_Float,		"ph_break_common_factor",		&ph_console::phBreakCommonFactor		,			0.f		,1000000000.f	);
	CMD4(CCC_Float,		"ph_rigid_break_weapon_factor",	&ph_console::phRigidBreakWeaponFactor	,			0.f		,1000000000.f	);
	CMD4(CCC_Integer,			"ph_tri_clear_disable_count",	&ph_console::ph_tri_clear_disable_count	,			0,		255				);
	CMD4(CCC_Float,		"ph_tri_query_ex_aabb_rate",	&ph_console::ph_tri_query_ex_aabb_rate	,			1.01f	,3.f			);
...
extern XRPHYSICS_API float	camera_collision_character_shift_z ;
	CMD4(CCC_Float,"camera_collision_character_shift_z", &camera_collision_character_shift_z,	0.f, 1.f );
	CMD4(CCC_Float,"camera_collision_character_skin_depth", &camera_collision_character_skin_depth, 0.f, 1.f	);
...
CMD4(CCC_Float,		"dbg_text_height_scale",	&dbg_text_height_scale	,			0.2f	,5.f	);
#endif
