...
#if defined(WIN32) || defined(WIN64) || defined(_XBOX) || defined(SN_TARGET_PS3)
...
#	include "sysdeps/win32/malloc-machine.h"
#elif defined(SN_TARGET_PS3)
#	include "sysdeps/pthread/malloc-machine.h"
#else // #elif defined(SN_TARGET_PS3)
#	error please include corresponding header here
#endif // #elif defined(SN_TARGET_PS3)

//#if defined(SN_TARGET_PS3)
//#	pragma comment( lib, "pthread")
//#endif // #if defined(SN_TARGET_PS3)
...
#define munmap_chunk(mst, p) do {                         \
  ...
  if (CALL_MUNMAP((char*)(p) - prevsize, (int)psize) == 0)     \
...
#define public_cALLOc    pt3calloc
#define public_fREe      pt3free
#define public_cFREe     pt3cfree
#define public_mALLOc    pt3malloc
#define public_mEMALIGn  pt3memalign
#define public_rEALLOc   pt3realloc
#define public_vALLOc    pt3valloc
#define public_pVALLOc   pt3pvalloc
#define public_pMEMALIGn pt3posix_memalign
#define public_mALLINFo  pt3mallinfo
#define public_mALLOPt   pt3mallopt
#define public_mTRIm     pt3malloc_trim
#define public_mSTATs    pt3malloc_stats
#define public_mUSABLe   pt3malloc_usable_size
#define public_iCALLOc   pt3independent_calloc
#define public_iCOMALLOc pt3independent_comalloc
#define public_gET_STATe pt3malloc_get_state
#define public_sET_STATe pt3malloc_set_state
...
struct malloc_arena {
  ...
  /* Space for mstate.  The size is just the minimum such that
     create_xray_mspace_with_base can be successfully called.  */
...
_int_new_arena( size_t size)
{
  ...
  a = CALL_MMAP( (int)mmap_sz );
  if ((char*)a == (char*)(size_t)-1)
    return 0;

  m = create_xray_mspace_with_base((char*)a + MSPACE_OFFSET,
...
malloc_starter( size_t sz, const void *caller)
{
  ...
  victim = xray_mspace_malloc(arena_to_mspace(&main_arena), sz);
...
memalign_starter( size_t align, size_t sz, const void *caller)
{
  ...
  /*ptmalloc_init_minimal();*/
  victim = xray_mspace_memalign(arena_to_mspace(&main_arena), align, sz);
  THREAD_STAT(++main_arena.stat_starter);
...
#define ATFORK_ARENA_PTR ((void*)(size_t)-1)
...
//    return xray_mspace_malloc(arena_to_mspace(&main_arena), sz);
//  } else {
...
//  xray_mspace_free(arena_to_mspace(ar_ptr), mem);
//  if(vptr != ATFORK_ARENA_PTR)
...
# endif /* USE_STARTER & 1 */
  ...
  mspace = create_xray_mspace_with_base((char*)&main_arena + MSPACE_OFFSET,
...
#if 0
  if (! secure) {
    if (!_dupenv_s(&s,&length,"MALLOC_TRIM_THRESHOLD_"))
      public_mALLOPt(M_TRIM_THRESHOLD, atoi(s));
    if (!_dupenv_s(&s,&length,"MALLOC_TOP_PAD_") || !_dupenv_s(&s,&length,"MALLOC_GRANULARITY_"))
      public_mALLOPt(M_GRANULARITY, atoi(s));
    if (!_dupenv_s(&s,&length,"MALLOC_MMAP_THRESHOLD_"))
...
public_mALLOc(size_t bytes)
{
  ...
  victim = xray_mspace_malloc(arena_to_mspace(ar_ptr), bytes);
...
public_rEALLOc(void* oldmem, size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  newp = xray_mspace_realloc(arena_to_mspace(ar_ptr), oldmem, bytes);
...
public_mEMALIGn( size_t alignment, size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  p = xray_mspace_memalign(arena_to_mspace(ar_ptr), alignment, bytes);
...
public_vALLOc( size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  p = xray_mspace_memalign(arena_to_mspace(ar_ptr), 4096, bytes);
...
#define HALF_INTERNAL_SIZE_T \
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  mem = xray_mspace_calloc(arena_to_mspace(ar_ptr), bytes, 1);
...
public_iCALLOc( size_t n, size_t elem_size, void* chunks[])
{
  ...
  m = xray_mspace_independent_calloc(arena_to_mspace(ar_ptr), n, elem_size, chunks);
...
public_iCOMALLOc( size_t n, size_t sizes[], void* chunks[])
{
  ...
  if (ar_ptr != &main_arena) {
    ...
    m_sizes = xray_mspace_malloc(arena_to_mspace(ar_ptr), n*sizeof(size_t));
    if (!m_sizes) {
      (void)mutex_unlock(&ar_ptr->mutex);
      return 0;
    }
    ...
    if (!chunks) {
      chunks = xray_mspace_malloc(arena_to_mspace(ar_ptr),
			     n*sizeof(void*)+FOOTER_OVERHEAD);
      if (!chunks) {
	xray_mspace_free(arena_to_mspace(ar_ptr), m_sizes);
	(void)mutex_unlock(&ar_ptr->mutex);
	return 0;
      }
  ...
  m = xray_mspace_independent_comalloc(arena_to_mspace(ar_ptr), n, m_sizes, chunks);
...
public_mTRIm( size_t s)
{
  ...
  (void)mutex_lock(&main_arena.mutex);
  result = xray_mspace_trim(arena_to_mspace(&main_arena), s);
...
{
  int result;
  result = xray_mspace_mallopt(p, v);
  return result;
}
...
public_mALLINFo(  )
{
  ...
  for (i=0, ar_ptr = &main_arena;; ++i) {
	result.uordblks += xray_mspace_mallinfo(arena_to_mspace(ar_ptr)).uordblks;
    ar_ptr = ar_ptr->next;
    if (ar_ptr == &main_arena)
      break;
  }
	...
	)
{
	...
	region->size							= (size_t)buffer_size;
	...
	result->total_size						= (size_t)buffer_size;
	result->free_size						= (size_t)buffer_size;
