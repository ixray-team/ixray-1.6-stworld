...
class XRNETSERVER_API IPureClient :	private MultipacketReciever,
...
//	void					SetClientID		(ClientID const& local_client) { net_ClientID = local_client; };
	...
	IC virtual	void		SendTo_LL				(void* data, u32 size, u32 dwFlags=DPNSEND_GUARANTEED );
...
public:
	...
	// send
	virtual	void			Send					(NET_Packet& P, u32 dwFlags=DPNSEND_GUARANTEED);
	virtual void			Flush_Send_Buffer		();
	virtual void			OnMessage				( const void* data, u32 size);
...
//	ClientID const &		GetClientID				() { return net_ClientID; };
	...
	IC BOOL					net_IsSyncronised		( ) const { return net_Syncronised; }
...
//	ClientID				net_ClientID;
	...
	void			OnDataReceived	( const void* data, u32 size );
	void			OnDataSent	( const void* data, u32 size );
};
