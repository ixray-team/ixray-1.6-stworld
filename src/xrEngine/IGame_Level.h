...
protected:
	// Network interface
	CObject*					pCurrentActor;
	CObject*					pCurrentViewActor;
...
public:
	...
	virtual shared_str			map_name				( ) const = 0;
	...
	virtual void				net_Update				( )										= 0;
			...
			BOOL				Load					( u32 dwNum );
	virtual BOOL				LoadLobbyMenu			( LPCSTR lobby_menu_name ) =0;
	...
	// Main interface
	ICF CObject*				CurrentActor			( void ) const							{ return pCurrentActor;				}
	ICF CObject*				CurrentViewActor		( void ) const							{ return pCurrentViewActor;			}
